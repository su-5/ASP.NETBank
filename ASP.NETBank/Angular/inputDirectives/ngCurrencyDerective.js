...
(function () {
    ...
    // account directive
    /* @ngInject */
    var ngCurrency = function ($filter, $locale) {
        return {
            require: 'ngModel',
            link: function (scope, element, attrs, controller) {

                var hardCap, min, max, currencySymbol, ngRequired = ['', 'true'].indexOf(attrs.ngRequired) !== -1;
                var active = true;
                var fraction = 4;


                $locale.NUMBER_FORMATS.DECIMAL_SEP = '.'; //Change separator everywhere to '.'

                attrs.$observe('ngCurrency',
                    function (value) {
                        active = (value !== 'false');
                        if (active) {
                            reformat();
                ...
                attrs.$observe('currencySymbol',
                    function (value) {
                        currencySymbol = value;
                        reformat();
                    });
                scope.$watch(attrs.ngRequired,
                    function (value) {
                        ngRequired = value;
                        revalidate();
                    });
                attrs.$observe('fraction',
                    function (value) {
                        fraction = value || 2;
                        reformat();
                        revalidate();
                ...
                controller.$formatters.push(function (value) {
                    if ([undefined, null, ''].indexOf(attrs.fraction) !== -1 && [undefined, null, ''].indexOf(value) === -1) {//если количество знаков после зпт не было задано
                        //отображаем столько знаков, сколько есть у числа
                        if (parseFloat(value).toString().split('.').length == 2) {
                            fraction = parseInt(parseFloat(value).toString().split('.')[1].length);
                        } else {
                            fraction = 0;
                        }
                    }
                ...
                function reformat() {
                    if (active) {
                        ...
                        if (controller.$options) {
                            ...
                            updateOn = controller.$options.getOption
                                ? controller.$options.getOption('updateOn')
                                : controller.$options.updateOn;
                        }
                        if (updateOn === 'blur') {
                ...
                function revalidate() {
                    ...
                    if (active) {
                        ...
                        if (value !== controller.$$rawModelValue) {
                            controller.$setViewValue(value.Round(fraction));
                ...
                element.bind('focus',
                    function () {
                        if (active) {
                            var value = [undefined, null, ''].indexOf(controller.$$rawModelValue) === -1
                                ? $filter('number')(controller.$$rawModelValue, fraction).replace(/$locale.NUMBER_FORMATS.GROUP_SEP/g, '')
                                : controller.$$rawModelValue;
                ...
                function clearRex(dChar) {
                    return RegExp('\\-{0,1}((\\' + dChar + ')|([0-9]{1,}\\' + dChar + '?))&?[0-9]{0,' + fraction + '}',
                        'g');
                }
                ...
                function clearValue(value) {
                    ...
                    var neg_regexp = RegExp('[0-9.' + '.' + $locale.NUMBER_FORMATS.GROUP_SEP + ']+');
    ...
    angular
        ...
        .directive("ngCurrency", ['$filter', '$locale', ngCurrency]);
})();